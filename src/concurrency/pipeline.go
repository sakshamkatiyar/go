...
func randomCounter(out chan int) {
	...
	var random int
	for x := 0; x < 10; x++ {
		random = rand.Intn(50) // randon number generation is ordered
		out <- random
	}
	close(out)
}
...
func generateFibonacci(out chan<- fibvalue, in <-chan int) { // specify channel direction
	defer wg.Done()
	var input float64
	// for {
	// 	v, ok := <-in
	...
	// 	}
	// }
	for v := range in { // receive data from channel
		input = float64(v)
